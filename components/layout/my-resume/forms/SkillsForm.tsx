...
const SkillsForm = ({ params }: { params: { id: string } }) => {
  ...
  const [skillsList, setSkillsList] = useState(
    formData.skills.length > 0
      ? formData.skills.map((skill: { rating: number; }) => ({
          ...skill,
        ...
        {skillsList.map((item: any, index: number) => (
          ...
          >
            ...
            <div className="flex items-center space-x-2">
              <label className="text-slate-700 font-semibold">Rating:</label>
              <select
                className="p-2 border rounded"
                value={item.rating || 1}
                onChange={(e) => handleChange(index, "rating", parseInt(e.target.value, 10))}
              >
                {[1, 2, 3, 4, 5].map((num) => (
